...
private fun ColumnScope.SeasonDisplayPage(
    ...
    LabeledCheckbox(
        label = stringResource(AMR.strings.action_display_unseen_badge),
    ...
    val displayModeItems = listOf(
        stringResource(MR.strings.show_title) to 0L,
        "Show season number" to Anime.SEASON_DISPLAY_MODE_NUMBER,
    )
    displayModeItems.map { (label, mode) ->
        RadioItem(
            label = label,
...
private fun SetAsDefaultDialog(
    ...
    AlertDialog(
        onDismissRequest = onDismissRequest,
        title = { Text(text = "Season settings") },
        text = {
            Column(
                ...
                LabeledCheckbox(
                    label = "Also set season settings for library",
